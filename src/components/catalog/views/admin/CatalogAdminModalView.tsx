...
export const CatalogAdminModalView: FC<CatalogAdminModalViewProps> = props =>
{
    ...
    return createPortal(
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-2" onClick={ onClose }>
            ...
            <div className={ `nitro-card-shell nitro-catalog-admin-modal relative flex max-h-[calc(100vh-16px)] ${ widthClassName } max-w-[calc(100vw-16px)] flex-col overflow-hidden shadow-lg` } onClick={ event => event.stopPropagation() }>
                ...
                <div className="nitro-catalog-admin-body flex min-h-0 flex-col gap-2.5 overflow-y-auto p-3">
                    { children }
                </div>
            </div>
        </div>,
