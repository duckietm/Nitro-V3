...
export const YouTubePlayerView: FC<{}> = () => {
    ...
    const [_spectators, setSpectators] = useState<{ id: number; name: string; look: string }[]>([]);
    const [broadcastVideo, setBroadcastVideo] = useState('');
    const [broadcastSender, setBroadcastSender] = useState('');
    const [_broadcastPlaylist, setBroadcastPlaylist] = useState<string[]>([]);
    ...
    const loadRoomUsers = () => {
        try {
            ...
            for (let i = 0; i < 500; i++) {
                ...
                if (userData?.name && userData.type === 1 && !seen.has(userData.userId)) {
    ...
    const roomSession = GetRoomSession();
    const isMyRoom = GetSessionDataManager().isModerator || roomSession?.isRoomOwner;
                            ...
                            {showVolumeSlider && (
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setVolumePreset(parseInt(e.target.value, 10));
                                    }}
                                    className="w-full"
                                />
                            )}
