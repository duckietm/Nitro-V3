...
const containerVariants = {
    ...
    exit: { transition: { staggerChildren: 0.01, staggerDirection: -1 as const } }
};

const itemVariants = {
    hidden: { opacity: 0, y: 10, scale: 0.8 },
    visible: { opacity: 1, y: 0, scale: 1, transition: { type: 'spring', stiffness: 400, damping: 22 } },
    exit: { opacity: 0, y: 4, scale: 0.9, transition: { duration: 0.08 } }
};
...
export const ToolbarView: FC<{ isInRoom: boolean }> = props =>
{
    ...
    return (
        <>
            ...
            { isInRoom &&
                <div className={ `fixed bottom-0 left-0 right-0 z-40 flex h-[52px] items-end px-0 pt-[2px] pb-0 pointer-events-none ${ desktopToolbarFrameClasses } ${ isToolbarOpen ? (hasDesktopUnifiedShell ? `${ desktopToolbarOpenClasses } rounded-t-[12px] border border-b-0 border-white/8 bg-[rgba(10,10,12,0.58)] shadow-[0_-6px_18px_rgba(0,0,0,0.18)]` : 'rounded-t-[12px] border border-b-0 border-white/8 bg-[rgba(10,10,12,0.58)] shadow-[0_-6px_18px_rgba(0,0,0,0.18)]') : `border-0 bg-transparent shadow-none ${ desktopToolbarOpenClasses }` }` }>
                    <motion.div
                        ...
                        animate={ { scale: isToolbarOpen ? 1 : 0.92 } }
                        whileHover={ { scale: 1.06 } }
                        whileTap={ { scale: 0.88 } }>
                        <motion.svg
                            animate={ { rotate: isToolbarOpen ? 180 : 0 } }
                            transition={ { type: 'spring', stiffness: 300, damping: 22 } }
                            className={ `h-3.5 w-3.5 text-white/70 ${ desktopToggleIconClasses }` }
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor">
            ...
            <div className={ `${ isToolbarOpen || !isInRoom ? '' : 'pointer-events-none' }` }>
                <motion.div
                    key="desktop-unified-shell"
                    initial={ false }
                    animate={ { opacity: isToolbarOpen || !isInRoom ? 1 : 0, scale: isToolbarOpen || !isInRoom ? 1 : 0.97, y: isToolbarOpen || !isInRoom ? 0 : 8 } }
                    transition={ { type: 'spring', stiffness: 300, damping: 28 } }
                    className={ `pointer-events-none fixed bottom-0 left-0 right-0 z-[39] h-[52px] rounded-t-[12px] border border-b-0 border-white/8 bg-[rgba(10,10,12,0.58)] shadow-[0_-6px_18px_rgba(0,0,0,0.18)] will-change-transform ${ desktopBlockClasses }` } />
